...
import { ArrowLeft, Package, Calendar, AlertCircle, Users, DollarSign, CheckCircle, XCircle } from 'lucide-react';
    ...
    const loadOrder = async () => {
        try {
            ...
            if (response.data.success) {
                ...
                console.log('Datos de la orden:', response.data.data); // Para depurar
            } else {
        ...
        <div className="p-8 max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <Card>
                    ...
                    <CardContent>
                        <p className="font-semibold">{order.technicalSheet?.name || 'No especificado'}</p>
                        <p className="text-sm text-gray-500 mt-1">Referencia: {order.technicalSheet?.reference || 'N/A'}</p>
                        <p className="text-sm text-gray-500">Composición: {order.technicalSheet?.composition || 'N/A'}</p>
                        <p className="text-sm text-gray-500">Peso: {order.technicalSheet?.weight || 'N/A'}</p>
                    </CardContent>
                </Card>
                ...
                <Card>
                    ...
                    <CardContent>
                        <p className="font-semibold">{order.technicalSheet?.client?.nombre || 'No asignado'}</p>
                        <p className="text-sm text-gray-500 mt-1">Empresa: {order.technicalSheet?.client?.empresa || 'N/A'}</p>
                    </CardContent>
                </Card>
            </div>

            {/* Cantidad y fase actual */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                ...
                <Card>
                    ...
                    <CardContent>
                        <p className="text-2xl font-bold">{order.current_phase?.nombre || 'No asignada'}</p>
                ...
                <Card>
                    ...
                    <CardContent>
                        <p className="text-2xl font-bold">{formatCurrency(order.technicalSheet?.estimated_cost)}</p>
                        <p className="text-sm text-gray-500">Costo real: {formatCurrency(order.actual_cost)}</p>
                    </CardContent>
                </Card>
            </div>
